...
[source.terminal]
----
bosh --ca-cert=/var/tempest/workspaces/default/root_ca_certificate --environment={{director_ip_address}} log-in
----
+
When prompted to log in, enter `director` for the user name, and the previously noted secret for the password.

. For convenience, let's alias that target environment as 'pcf':
+
[source.terminal]
----
